...
from legal_agent.dialogue.intake import ALL_FIELD_KEYS, NOISE_CHECKLIST

# Human-readable field spec, derived from the SAME checklist the rule-based path
# uses, so both collect an identical fact set.
_FIELD_SPEC = "\n".join(
...
INTAKE_SYSTEM_PROMPT = (
    ...
    "【何時結束】當六個欄位都大致問到、或使用者表示沒有更多資訊時,把 ready 設為 true。\n"
    "【輸出格式(務必嚴格遵守)】只輸出一個 JSON,放在 ```json 與 ``` 之間,前後不要有"
    "其他文字:\n"
    '```json\n{"reply": "你要對使用者說的話(含追問)", '
    '"facts": {"noise_type": "…", "timing": "…"}, "ready": false}\n```\n'
...
def build_intake_prompt(history: list[dict], facts: dict) -> str:
...
def parse_intake_response(text: str, prev_facts: dict) -> IntakeTurn:
    ...
    than crash. Only known field keys are merged; previously-known facts persist."""
    raw = None
    ...
    if isinstance(new, dict):
        for k, v in new.items():
            if k in ALL_FIELD_KEYS and isinstance(v, str) and v.strip():
                facts[k] = v.strip()
...
def run_smart_intake_turn(history: list[dict], facts: dict, llm) -> IntakeTurn:
    ...
    return parse_intake_response(llm(build_intake_prompt(history, facts)), facts)
